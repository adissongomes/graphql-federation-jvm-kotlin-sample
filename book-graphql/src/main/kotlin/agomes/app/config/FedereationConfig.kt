...
import agomes.domain.book.AuthorDTO
import agomes.domain.book.Book
import agomes.domain.book.BookDTO
...
class FedereationConfig {
    ...
    fun schemaParserDictionary(): SchemaParserDictionary? {
        return SchemaParserDictionary()
            .add("Book", BookDTO::class.java)
            .add("Author", AuthorDTO::class.java)
    }
    ...
    @Bean
    fun graphQLSchema(schemaTransformer: SchemaTransformer): GraphQLSchema? {
        return schemaTransformer
            .fetchEntities {
                it.getArgument<List<Map<String, Any>>>(_Entity.argumentName).map {
                    if ("Author" == it["__typename"])
                        return@map AuthorDTO(it["id"].toString().toInt())
                    return@map null
                }
            }
